 ...
 * @author: hch
 */
export function getSystem(ctx) {
  let scale = ctx.width / 375 //按照苹果留 375*667比例 其他型号手机等比例缩放 显示
  // console.log({ w: ctx.width, h: ctx.height, scale: 1 })
  return { w: ctx.width, h: ctx.height, scale: scale }
}
...
export function drawTextReturnH(
  ...
  if (textAlign) {
    ...
    switch (textAlign) {
      ...
      case 'right':
        x = (getSystem(ctx).w - maxWidth) / 2 + maxWidth
        break

      default:
        // 左对齐
        x = (getSystem(ctx).w - maxWidth) / 2
        break
    }
  }
